package com.idx.launcher.takeout.ui;
...
import com.idx.launcher.takeout.SaleManager;
import com.idx.launcher.takeout.adapter.TakeoutCarAdapter;
import com.idx.launcher.takeout.data.item.ImoranTMResponse;
import com.idx.launcher.takeout.data.item.TMContent;
import com.idx.launcher.takeout.data.item.TMSummary;
import com.idx.launcher.takeout.data.item.TakeoutMenu;
import com.idx.launcher.takeout.data.room.TakeoutDataSource;
import com.idx.launcher.takeout.data.room.TakeoutInjection;
import com.idx.launcher.takeout.data.room.TakeoutRepository;
import com.idx.launcher.takeout.data.room.TakeoutSeller;
import com.idx.launcher.takeout.utils.Constant;
import com.idx.launcher.takeout.utils.ImoranResponseParseUtil;
...
//        setPresentData();
    ...
    private void setScenes(String json) {
        ...
        try {
            ...
            VUIDataEntity.wrapData(((LauncherApplication) getApplication()).getDataEntity(), mPageId, mQueryId);
    ...
    private void query() {
        mTakeoutRepository.queryAllFood(mSellerName, new TakeoutDataSource.LoadAllSellerCallback() {
            ...
            public void onSuccess(List<TakeoutSeller> sellers) {
                ((LauncherApplication) getApplication()).getDataEntity().setPresent_data(null);
                ...
                ((LauncherApplication) getApplication()).getDataEntity().setPresent_data(mCartReportBean);
        ...
        if (!mFlag) {
            ...
            try {
                ...
                if (type.equals("takeoutmenucart")){
                    ...
                    try {
                        JSONArray array = jsonObject.getJSONObject("data").getJSONObject("content").getJSONObject("reply").getJSONArray("takeoutmenucart");
                        if (array!=null && mSellers!=null){
                            if (mSellers.size()==array.length()){
                                TTSManager.getInstance(getBaseContext()).speak(tts, true);
                                modifyGoodCount(json);
                            }else {
                                TTSManager.getInstance(getBaseContext()).speak("已超出购物车当前菜品数量,请重试!", true);
                            }
                        }
                    }catch (JSONException e){
                        Toast.makeText(mContext, "数据有误,请重试!", Toast.LENGTH_SHORT).show();
                    }
