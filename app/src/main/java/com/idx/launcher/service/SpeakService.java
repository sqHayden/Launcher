...
import com.idx.launcher.LauncherActions;
import com.idx.launcher.LauncherApplication;
import com.idx.launcher.R;
import com.idx.launcher.calendar.CalenderActivity;
import com.idx.launcher.data.JsonData;
import com.idx.launcher.data.JsonUtil;
import com.idx.launcher.dish.DishActivity;
import com.idx.launcher.figure.FigureActivity;
import com.idx.launcher.home.HomeActivity;
import com.idx.launcher.imoran.ImoranManager;
import com.idx.launcher.imoran.TTSManager;
import com.idx.launcher.map.MapActivity;
import com.idx.launcher.music.MusicActivity;
import com.idx.launcher.music.utils.ToastUtil;
import com.idx.launcher.news.NewsActivity;
import com.idx.launcher.service.listener.DataListener;
import com.idx.launcher.service.listener.SpeakWzRecordListener;
import com.idx.launcher.takeout.CartReportBean;
import com.idx.launcher.takeout.ui.TakeoutActivity;
import com.idx.launcher.takeout.ui.TakeoutCarActivity;
import com.idx.launcher.takeout.ui.TakeoutSellerActivity;
import com.idx.launcher.takeout.utils.Constant;
import com.idx.launcher.user.hx.EaseLoginActivity;
import com.idx.launcher.user.iom.login.LoginActivity;
import com.idx.launcher.user.personal_center.Personal_center;
import com.idx.launcher.user.personal_center.order.OrderTimeActivity;
import com.idx.launcher.utils.AdjustVolume;
import com.idx.launcher.utils.MapUtils;
import com.idx.launcher.utils.MathTool;
import com.idx.launcher.utils.NetStatusUtils;
import com.idx.launcher.utils.SharedPreferencesUtil;
import com.idx.launcher.video.ui.VideoActivity;
import com.idx.launcher.video.ui.VideoDetailActivity;
import com.idx.launcher.videocall.VideoIntent;
import com.idx.launcher.videocall.call.data.CallDataSource;
import com.idx.launcher.videocall.call.data.CallInjection;
import com.idx.launcher.videocall.call.data.CallRepository;
import com.idx.launcher.videocall.call.data.MissedCall;
import com.idx.launcher.videocall.friend.AgreeFriendListener;
import com.idx.launcher.videocall.friend.DeleteFriendListener;
import com.idx.launcher.videocall.friend.FriendAct;
import com.idx.launcher.videocall.friend.RejectFriendListener;
import com.idx.launcher.videocall.friend.data.Friend;
import com.idx.launcher.videocall.friend.data.FriendDataSource;
import com.idx.launcher.videocall.friend.data.FriendInjection;
import com.idx.launcher.videocall.friend.data.FriendRepository;
import com.idx.launcher.videocall.utils.CallUtil;
import com.idx.launcher.videocall.utils.SpUtils;
import com.idx.launcher.weather.WeatherActivity;
import com.idx.launcher.wenzhi.ASRManager;
import com.idx.launcher.wenzhi.SpeakDialog;
                        ...
                        mHandler.postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (result.equals("选择方案一。")) {
                                    ...
                                    VUIDataEntity vuiDataEntity = ((LauncherApplication) getApplication()).getDataEntity();
                                    ...
                                    ImoranManager.getInstance(getBaseContext()).tell(result, ((LauncherApplication) getApplication()).getDataEntity(), mTellCallback);
                                }
                            }
                        }, 1500);
                    }
    ...
    private void handleMoranResponse(String json) {
        ...
        switch (domain) {
            ...
            case LauncherActions.Video.TARGET_VIDEO:
                ...
                break;
            case LauncherActions.Dish.TARGET_DISH:
                startTarget(DishActivity.class, json);
                break;
            case LauncherActions.Map.TARGET_MAP:
            case LauncherActions.Map.TARGET_RESTAURANT:
            case LauncherActions.Map.TARGET_VIEWSPOT:
                startTarget(MapActivity.class, json);
                break;
            case LauncherActions.TakeOut.TARGET_TAKEOUT:
                if (type.equals(LauncherActions.TakeOut.TARGET_TYPE_SHOP)) {
                    startTarget(TakeoutActivity.class, json);
                } else if (type.equals(LauncherActions.TakeOut.TARGET_TYPE_MENU)) {
                    startTarget(TakeoutSellerActivity.class, json);
                }
                break;
            case LauncherActions.TakeOut.TARGET_TAKEOUT_CAR:
//                !(topActivity().contains("LinQuConfirmOrderActivity") || topActivity().contains("PayOrderActivity"))
                ...
                break;
            case LauncherActions.Figure.TARGET_FIGURE:
                startTarget(FigureActivity.class, json);
                break;
            case LauncherActions.News.TARGET_NEWS:
                startTarget(NewsActivity.class, json);
                break;
            case LauncherActions.Phone.TARGET_PHONE:
                CallUtil.openVideoCall(getApplicationContext(), json);
                break;
            case LauncherActions.Cmd.TARGET_CMD:
                cmdType(type, json);
                break;
            case LauncherActions.Calendar.TARGET_CALENDAR:
                startTarget(CalenderActivity.class, json);
                break;
            case LauncherActions.Order.TARGET_ORDER_LIST:
    ...
    public class SpeakBinder extends Binder implements IService {
        ...
        public void requestData(String text, NLIRequest.onRequest callback) {
            ImoranManager.getInstance(getBaseContext()).tell(text, ((LauncherApplication) getApplication()).getDataEntity(), callback);
        }
    ...
    private void startTarget(Class<?> clz, String json) {
        if (!topActivity().contains("VideoCallActivity")) {
            if (isActivityRunning(clz.getName())) {
                ...
                if (clz.getName().equals(HomeActivity.class.getName())) {
            ...
            public void onContactDeleted(final String s) {
                ...
                if (matcher.find()) {
                    ...
                    mRepository.deleteFriend(s, new FriendDataSource.DeleteFriendSuccessCallback() {
                        @Override
                        public void onSuccess() {
                            if (topActivity().contains("FriendAct")) {
